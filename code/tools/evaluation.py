...
def parse_args():
    ...
    parser.add_argument("--data_path", metavar="DIR",
                        help="path to dataset", default='example_data/ScanNet/scans_test')
    parser.add_argument("--gt_path", metavar="DIR",
                        help="path to raw dataset", default='example_data/ScanNet/gt_mesh')
...
def process(scene, total_scenes_index, total_scenes_count):
    ...
    test_framid_ = os.listdir(os.path.join(args.data_path, scene, 'color'))
    test_framid = []
    for filename in test_framid_:
        if filename.endswith(".jpg"):
            test_framid.append(filename)
    ...
    # re-fuse to remove hole filling since filled holes are penalized in mesh metrics
    ...
    for i, (cam_pose, depth_trgt, _) in enumerate(dataloader):
        ...
        if i == 0:
            ...
            metrics_depth = {key: value + nan_process(value, temp[key], i)
